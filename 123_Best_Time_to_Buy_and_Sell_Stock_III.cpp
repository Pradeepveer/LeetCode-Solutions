...
public:
    ...
    int f1(int ind, int buy, int transactions, vector<int>& prices){
        ...
        int profit = 0;
        if (buy){
            int take = -prices[ind] + f1(ind+1, 0, transactions - 1, prices);
    ...
    int maxProfit(vector<int>& prices){
        int n = prices.size();
        vector<vector<int>> next(2, {0, 0, 0}), cur(2, {0, 0, 0});
        // for base case initial values are already zero so no need to write base case again
    ...
    int maxProfit(vector<int>& prices) {
        int n = prices.size();
        // recursion
        // return f1(0, 1, 4, prices);

        // memoization
        vector<vector<vector<int>>> dp(n, vector<vector<int>>(2, vector<int>(3, -1)));
        return f2(0, 1, 2, prices, dp);
    }
};